...
class ChatServerDispatcher extends ServerDispatcher<ServerMessage>
{
	...
	private NotificationDispatcher<SendNotification,ReceiveNotificationThread,ReceiveNotificationThreadCreator> sendFileDispatcher; //同步
	...
	public ChatServerDispatcher(int threadPoolSize, long threadKeepAliveTime, int schedulerPoolSize, long schedulerKeepAliveTime)
	{
		...
		this.sendFileDispatcher = new NotificationDispatcher.NotificationDispatcherBuilder<SendNotification, ReceiveNotificationThread, ReceiveNotificationThreadCreator>()
				.poolSize(ServerConfig.NOTIFICATION_DISPATCHER_POOL_SIZE)
				.threadCreator(new ReceiveNotificationThreadCreator())
				.notificationQueueSize(ServerConfig.NOTIFICATION_QUEUE_SIZE)
				.dispatcherWaitTime(ServerConfig.NOTIFICATION_DISPATCHER_WAIT_TIME)
				.waitRound(ServerConfig.NOTIFICATION_DISPATCHER_WAIT_ROUND)
				.idleCheckDelay(ServerConfig.NOTIFICATION_DISPATCHER_IDLE_CHECK_DELAY)
	...
	public void dispose(long timeout) throws InterruptedException
	{
		...
		this.sendFileDispatcher.dispose();
	...
	public void process(OutMessageStream<ServerMessage> message)
	{
		...
		{	
			...
			case ChatMessageType.PEER_CHAT_NOTIFICATION:
				...
				case ChatMessageType.SEND_FILE_NOTIFICATION:
				System.out.println("SEND_FILE_NOTIFICATION received @" + Calendar.getInstance().getTime());
				// Check whether the chatting notification dispatcher is ready or not. 02/15/2016, Bing Li
				if (!this.sendFileDispatcher.isReady())
				{
					// Execute the notification dispatcher concurrently. 02/15/2016, Bing Li
					super.execute(this.sendFileDispatcher);
				}
				// Enqueue the instance of ChatNotification into the dispatcher for concurrent processing. 02/15/2016, Bing Li
				this.sendFileDispatcher.enqueue((SendNotification) message.getMessage());
				break;
		}
	}
}
